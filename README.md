...
- [X] Design the data model and use _knex migrations_ to create the database and tables needed to satisfy the following business rules:
  - [X] a `project` can have multiple `tasks`. 1 to many
  - [X] a `task` belongs to only one `project`. 1 to many--foreign key
  - [X] a `project` can use multiple `resources`. Example of `resources` are: computer, conference room, microphone, delivery van. 1 to many
  - [X] the same `resource` can be used in multiple `projects`. many to many
